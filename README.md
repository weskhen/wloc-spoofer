...
| 检测方式 | 说明 | 风险等级 |
|---------|------|---------|
| 证书检测 | App 检查当前连接是否使用非 Apple 根证书 | 高（银行、金融类 App 常做） |
| WLOC 与 GPS 不一致 | 室外 GPS 信号强时，真实 GPS 坐标与 WLOC 返回坐标不匹配 | 高 |
| IP 归属地不匹配 | IP 地址的地理位置与定位坐标差异大 | 中 |
| 代理设置检测 | App 读取系统 Wi-Fi 代理配置 | 低 |
| 定位突变 | 短时间内位置发生不可能的跳跃 | 低 |
### 降低检测风险的建议
- **室内使用**：GPS 信号弱时 iOS 只依赖 WLOC，无 GPS 交叉验证
- 配合 VPN 伪装 IP 归属地
- 配置代理后关闭再开启定位服务（清除缓存）
- 避免在银行、支付等严格 App 上使用
